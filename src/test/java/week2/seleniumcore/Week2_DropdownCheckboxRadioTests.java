...
public class Week2_DropdownCheckboxRadioTests {
    ...
    // ─────────────────────────────────────────────
    // TEST 5 FIX:
    // Root cause: JS click doesn't fire React's synthetic
    // mouse events — dropdown opens but React state doesn't
    // update so options never render.
    // Fix: Use Actions.click() which fires real mouse events
    // that React's event system can intercept.
    // ─────────────────────────────────────────────
    @Test(description = "Verify custom React Select dropdown using 2-step click and explicit wait")
    public void test5_CustomDropdown_TwoStepClick() throws InterruptedException {
        driver.get("https://demoqa.com/select-menu");

        // ── Sub-test A: withOptGroup React Select ──

        // Step 1 — Scroll then use Actions click (fires real mouse events for React)
        WebElement groupDropdown = wait.until(ExpectedConditions
            .elementToBeClickable(By.id("withOptGroup")));
        js.executeScript("arguments[0].scrollIntoView({block:'center'});", groupDropdown);
        actions.moveToElement(groupDropdown).click().perform();

        // Step 2 — Wait for React to render options into DOM
        // React Select renders options inside a menu portal
        // Use any visible text-bearing div after dropdown opens
        WebElement groupOption = wait.until(ExpectedConditions
            .visibilityOfElementLocated(By.xpath(
                "//*[contains(text(),'Group 1, option 2')]")));
        actions.moveToElement(groupOption).click().perform();

        // Verify — React Select updates the control div with selected text
        WebElement groupControl = wait.until(ExpectedConditions
            .visibilityOfElementLocated(
                By.cssSelector("#withOptGroup > div")));
        Assert.assertTrue(groupControl.getText().contains("Group 1, option 2"),
            "Should show selected. Actual: " + groupControl.getText());
        System.out.println("Sub-test A PASSED: " + groupControl.getText());

        // ── Sub-test B: selectOne title dropdown ──

        // Step 1 — Scroll and Actions click
        WebElement titleDropdown = wait.until(ExpectedConditions
            .elementToBeClickable(By.id("selectOne")));
        js.executeScript("arguments[0].scrollIntoView({block:'center'});", titleDropdown);
        actions.moveToElement(titleDropdown).click().perform();

        // Step 2 — Wait for options to appear, then find "Dr." by text
        String target = "Dr.";
        WebElement drOption = wait.until(ExpectedConditions
            .visibilityOfElementLocated(
                By.xpath("//*[normalize-space(text())='" + target + "']")));

        System.out.println("Found option: '" + drOption.getText()
            + "' tag=" + drOption.getTagName()
            + " class=" + drOption.getAttribute("class"));

        actions.moveToElement(drOption).click().perform();

        // Verify
        WebElement titleControl = wait.until(ExpectedConditions
            .visibilityOfElementLocated(
                By.cssSelector("#selectOne > div")));
        Assert.assertTrue(titleControl.getText().contains(target),
            "Should show Dr. Actual: " + titleControl.getText());

        System.out.println("TEST 5 PASSED — Custom dropdown 2-step click verified");
    }
}